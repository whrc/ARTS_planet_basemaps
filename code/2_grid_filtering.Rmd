...
arts_grids = planet_grids_filter |>
  ...
  mutate(
    ...
    # having NA in start_year doesn't work with line 4 of target_year = case_when(...), so switch to 0
...
# st_write(
#   arts_grids,
#   './data/rts_polygon_basemap_correspondence_v.3.1.0.geojson'
#   )
...
basemap_years = arts_grids_filter |>
  pull(planet_basemap_year) |>
  unique()
...
```{r}
st_write(arts_grids_filter, './data/planet_grids_for_arts_closest_year_v.3.1.0.geojson')
```

# Additional Imagery Since ARTS v.2.1.0

```{r}
arts_grid_filter_v.2.1.0 = st_read("./data/planet_grids_for_arts_closest_year_v.2.1.0.geojson")
arts_grid_filter_v.3.1.0 = st_read("./data/planet_grids_for_arts_closest_year_v.3.1.0.geojson")
```

```{r}
new_grids = anti_join(
  arts_grid_filter_v.3.1.0, 
  arts_grid_filter_v.2.1.0 |>
    st_drop_geometry()
)
...
# st_write(
#   new_grids,
#   "./data/new_planet_grids_for_arts_closest_year_v.3.1.0.geojson"
# )
```

```{r}
new_grids_bbox = st_bbox(new_grids)
new_grids_xlim = c(new_grids_bbox[c("xmin", "xmax")])
new_grids_ylim = c(new_grids_bbox[c("ymin", "ymax")])

ggplot() +
  geom_sf(data = world) +
  geom_sf(data = new_grids,
          aes(
...
## Which Grids Are Required Adjacent Grids (Not New Data in v.3.1.0)

```{r}
arts_v.2.1.0 = st_read('../ARTS/ARTS_main_dataset/v.2.1.0/ARTS_main_dataset_v.2.1.0.geojson')
```
...
new_rts_tiles = new_rts |>
  st_buffer(
    dist = 128 * 4.77, # 256 pixel sides
    endCapStyle = "SQUARE"
  )

new_adjacent_grids = new_grids %>%
  filter(
    !(
      st_intersects(., new_rts_tiles) %>%
        map_lgl(
          ~ length(.x) > 0
        )
    )
...
Apparently, none of the added grids were due to missing adjacent imagery. They were all due to added RTS in ARTS v.3.1.0.